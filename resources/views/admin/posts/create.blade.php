    ...
    <div class="bg-white rounded-xl shadow-sm border border-border-custom/30 overflow-hidden">
        <div class="px-6 py-4 bg-bg-secondary/40 border-b border-border-custom/20">
            <h3 class="font-serif font-bold text-sm text-primary">
                <i class="fas fa-edit text-secondary mr-2"></i> SOẠN THẢO NỘI DUNG BÀI VIẾT MỚI
            </h3>
        </div>
        ...
        <form method="POST" action="{{ route('admin.posts.store') }}" enctype="multipart/form-data" class="p-6 space-y-6">
            ...
            <div>
                ...
                <textarea 
                    ...
                    rows="12" 
                    required
                    placeholder="Nhập nội dung bài viết, sử dụng các thẻ HTML như <p>, <h2>, <strong> để trình bày..."
            ...
            <!-- Image upload & Status Grid -->
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <!-- Image upload -->
                <div>
                    <label for="featured_image" class="block text-xs font-bold text-text-primary uppercase tracking-wider mb-2">
                        Hình ảnh tiêu đề (Thumbnail) <span class="text-text-secondary/60">(Tối đa 2MB, JPG/PNG)</span>
                    </label>
                    <input 
                        type="file" 
                        name="featured_image" 
            ...
            <div class="pt-4 border-t border-border-custom/20 flex space-x-2">
                ...
                >
                    <i class="fas fa-save mr-1.5"></i> Lưu bài viết
                </button>
                <a 
