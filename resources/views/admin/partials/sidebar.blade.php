<!-- Desktop Sidebar -->
<div class="hidden lg:flex w-64 bg-bg-dark text-text-light flex-col flex-shrink-0 border-r border-primary/20">
    <!-- Sidebar Brand -->
...
<div id="admin-sidebar-backdrop" class="fixed inset-0 bg-black/60 z-40 hidden backdrop-blur-xs transition-opacity duration-300 lg:hidden"></div>

<!-- Mobile Slide-over Sidebar Drawer -->
<div id="admin-sidebar-mobile" class="fixed inset-y-0 left-0 z-50 w-64 bg-bg-dark text-text-light flex flex-col justify-between border-r border-primary/20 transform -translate-x-full hidden transition-transform duration-300 ease-in-out lg:hidden shadow-2xl">
    <div>
        <div class="h-16 flex items-center justify-between px-6 bg-black/30 border-b border-primary/10">
            <a href="{{ route('admin.dashboard') }}" class="flex items-center space-x-2">
                <span class="text-lg font-bold font-serif text-white tracking-wider">
                    Hoa Lư <span class="text-secondary">Admin</span>
                </span>
            </a>
            <button id="admin-sidebar-close" type="button" class="text-text-light/75 hover:text-white p-2 focus:outline-none">
                <i class="fas fa-times text-lg"></i>
            </button>
        </div>

        <nav class="py-6 px-4 space-y-1 overflow-y-auto max-h-[calc(100vh-8rem)]">
            <a 
                href="{{ route('admin.dashboard') }}" 
                class="flex items-center px-4 py-3 rounded-lg text-xs font-semibold tracking-wider transition-all {{ request()->routeIs('admin.dashboard') ? 'bg-primary text-white shadow-md' : 'text-text-light/75 hover:bg-white/5 hover:text-white' }}"
            >
