...
@section('title', 'Sửa Trang Thực Đơn')
@section('page_title', 'Chỉnh sửa trang Menu')
    ...
    <div class="bg-white rounded-xl shadow-sm border border-border-custom/30 overflow-hidden">
        <div class="px-6 py-4 bg-bg-secondary/40 border-b border-border-custom/20">
            <h3 class="font-serif font-bold text-sm text-primary">
                <i class="fas fa-edit text-secondary mr-2"></i> CHỈNH SỬA THÔNG TIN TRANG THỰC ĐƠN
            </h3>
        </div>

        <form method="POST" action="{{ route('admin.menu-boards.update', $menuBoard->id) }}" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input 
                    ...
                    name="title" 
                    id="title" 
                    value="{{ old('title', $menuBoard->title) }}" 
            ...
            <div>
                <span class="block text-xs font-bold text-text-primary uppercase tracking-wider mb-2">Hình ảnh hiện tại</span>
                <div class="w-32 h-40 rounded-lg overflow-hidden border border-border-custom/40 bg-bg-secondary/20 flex items-center justify-center">
                    @if($menuBoard->image)
                        <img src="{{ asset('storage/' . $menuBoard->image) }}" alt="{{ $menuBoard->title ?: 'Menu page' }}" class="w-full h-full object-cover">
                    @else
                        <span class="text-text-secondary text-[10px]">Chưa có ảnh</span>
                    @endif
                </div>
            </div>
            ...
            <div>
                <label for="image" class="block text-xs font-bold text-text-primary uppercase tracking-wider mb-2">
                    Thay đổi ảnh <span class="text-text-secondary/60">(Để trống nếu giữ nguyên ảnh cũ)</span>
                </label>
                <input 
                    type="file" 
                    name="image" 
            ...
            <div>
                ...
                <input 
                    ...
                    id="sort_order" 
                    min="0"
                    value="{{ old('sort_order', $menuBoard->sort_order) }}"
                    class="w-full px-4 py-2.5 rounded-lg border border-border-custom bg-bg-primary/20 text-text-primary focus:outline-none focus:ring-2 focus:ring-primary/25 focus:border-primary transition-all text-xs"
                >
                @error('sort_order')
                    <span class="text-error text-xs mt-1 block">{{ $message }}</span>
            ...
            <div class="flex items-center">
                <label class="flex items-center text-xs font-semibold text-text-primary cursor-pointer">
                    <input 
                        ...
                        name="is_active" 
                        value="1" 
                        {{ old('is_active', $menuBoard->is_active) ? 'checked' : '' }}
            ...
            <div class="pt-4 border-t border-border-custom/20 flex space-x-2">
                ...
                >
                    <i class="fas fa-save mr-1.5"></i> Cập nhật trang thực đơn
                </button>
                <a 
